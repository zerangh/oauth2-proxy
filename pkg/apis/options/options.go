...
func NewOptions() *Options {
	return &Options{
		...
		// Refresh cookie 1 hour before expiry to avoid session drops
		CookieRefresh:  23 * time.Hour,
		// Default to Lax to allow OAuth redirect flows while still providing CSRF protection
		CookieSameSite: "lax",
		CookieSecure:   true,
		CookieHTTPOnly: true,
		PingPath:       "/ping",
